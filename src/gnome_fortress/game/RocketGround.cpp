...
const model::PointSet *RocketGround::getPointSet() const {
    return pointSet;
}

RocketGroundTechnique *RocketGround::getTechnique() const {
    return technique;
}
...
model::PointSet *RocketGround::CreateSphereParticles(int num_particles) {
    ...
    // Create resource
    return new model::PointSet("Rocket Ground", vbo, num_particles, GL_POINTS);
}
