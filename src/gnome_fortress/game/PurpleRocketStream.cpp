...
PurpleRocketStream::~PurpleRocketStream() {
    glDeleteBuffers(1, &pointSet->vbo);
    delete pointSet;
}

void PurpleRocketStream::setPower(float p) {
    power = p;
}

void PurpleRocketStream::onDrawSelf(const glm::mat4 &parent_transform, unsigned int pass) const {
...
const model::PointSet *PurpleRocketStream::getPointSet() const {
    return pointSet;
}

PurpleRocketStreamTechnique *PurpleRocketStream::getTechnique() const {
    return technique;
}

model::PointSet *PurpleRocketStream::CreateConeParticles(int num_particles) {
