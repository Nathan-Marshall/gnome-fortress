...
namespace game {
    ...
    RocketGroundTechnique::RocketGroundTechnique(GLuint program)
...
void RocketGroundTechnique::setTimer(float time) {
    timer = time;
}

void RocketGroundTechnique::setPower(float p) {
    power = p;
}

void RocketGroundTechnique::bindMainTexture(const model::Texture *texture) {
    glActiveTexture(GL_TEXTURE0); // activate the first texture
    bindTexture(texture);
}

void RocketGroundTechnique::bindTexture(const model::Texture *texture) {
    if (texture) {
