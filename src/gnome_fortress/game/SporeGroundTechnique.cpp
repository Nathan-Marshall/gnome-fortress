...
SporeGroundTechnique::SporeGroundTechnique(GLuint program)
...
void SporeGroundTechnique::setTimer(float time) {
    timer = time;
}

void SporeGroundTechnique::setPower(float p) {
    power = p;
}

void SporeGroundTechnique::bindMainTexture(const model::Texture *texture) {
    glActiveTexture(GL_TEXTURE0); // activate the first texture
    bindTexture(texture);
}

void SporeGroundTechnique::bindTexture(const model::Texture *texture) {
    if (texture) {
