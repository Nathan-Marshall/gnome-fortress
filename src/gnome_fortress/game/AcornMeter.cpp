...
AcornMeter::AcornMeter(const Acorns *acorns, resource::ResourceManager &resourceManager, GLuint vbo, renderer::SpriteTechnique *technique)
    : acorns(acorns),
      acornIcons(Acorns::NUM_ACORNS) {

    for (int i = 0; i < Acorns::NUM_ACORNS; i++) {
        ui::SpriteNode *icon = new ui::SpriteNode(resourceManager.getOrLoadTexture(resources::textures::ui_acorn), vbo, technique);
        acornIcons[i] = icon;
