...
namespace game {
    ...
    PurpleRocketStreamTechnique::PurpleRocketStreamTechnique(GLuint program)
...
void PurpleRocketStreamTechnique::setTimer(float time) {
    timer = time;
}

void PurpleRocketStreamTechnique::setPower(float p) {
    power = p;
}

void PurpleRocketStreamTechnique::bindMainTexture(const model::Texture *texture) {
    glActiveTexture(GL_TEXTURE0); // activate the first texture
    bindTexture(texture);
}

void PurpleRocketStreamTechnique::bindTexture(const model::Texture *texture) {
    if (texture) {
